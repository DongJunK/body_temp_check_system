...
class MyApp:
    ...
    def click_search(self):
        ...
        conn = sqlite3.connect('database.db')  # 파일과 연결
        c = conn.cursor()  # 커서 설정
        c.execute("SELECT raspberry_pi_serial_Number, student_Number, LogTime, temperature FROM temp")  # temp
        ...
        if '선택' != self.combobox.get():
            ...
            if ' 의심환자' == self.combobox.get():
                c.execute(
                    "SELECT raspberry_pi_serial_Number, student_Number, LogTime, temperature FROM temp WHERE temperature>=38")
                tuples = c.fetchall()
                for row in tuples:
                    self.treeview.insert("", END, values=row)
